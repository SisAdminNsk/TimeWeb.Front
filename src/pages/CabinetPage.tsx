...
export const formatDateTime = (dateString: string): string => {
  if (!dateString) return '—';
  return new Date(dateString).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' });
};
...
const createStyles = (theme: any, isMobile: boolean) => {
  ...
  return {
    ...
    infoGrid: { display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(280px, 1fr))', gap: isMobile ? spacing.md : spacing.lg } as React.CSSProperties,
    infoItem: { marginBottom: 0 } as React.CSSProperties,
    infoLabel: { display: 'block', fontSize: typography.fontSize.xs, color: colors.gray500, marginBottom: spacing.xs, textTransform: 'uppercase' as const, letterSpacing: '0.5px', fontWeight: typography.fontWeight.semibold } as React.CSSProperties,
...
const ProfileSection: React.FC<{
  userData: any; isEditing: boolean; formData: any; emailError: string; isEmailTouched: boolean; isSaving: boolean;
  styles: ReturnType<typeof createStyles>;
  onEdit: () => void; onCancel: () => void; onSave: () => void;
  onInput: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void;
  onEmailBlur: () => void; setIsEmailTouched: (v: boolean) => void;
}> = ({ userData, isEditing, formData, emailError, isSaving, styles, onEdit, onCancel, onSave, onInput, onEmailBlur, setIsEmailTouched }) => {
  const isSaveDisabled = isEditing && (!isValidEmail(formData.email) || emailError !== '');
  return (
    <div style={styles.sectionCard}>
      <div style={styles.sectionHeader}>
        <h2 style={styles.sectionTitle}>Данные</h2>
        <p style={styles.sectionDesc}>Основная информация о вашем аккаунте</p>
      </div>
      <div style={styles.sectionBody}>
        <div style={styles.infoGrid}>
          <div style={styles.infoItem}>
            <label style={styles.infoLabel}>Имя пользователя</label>
            <div style={styles.fieldDisabled}>{userData?.name || 'Не указано'}</div>
          </div>
          <div style={styles.infoItem}>
            <label style={styles.infoLabel}>Email</label>
            <input type="email" name="email" value={isEditing ? formData.email : (userData?.email || 'Не указан')} onChange={onInput} onBlur={onEmailBlur} readOnly={!isEditing} placeholder="example@mail.com" style={isEditing && emailError ? styles.fieldError : isEditing ? styles.fieldInput : styles.fieldDisabled} onFocus={(e) => { e.currentTarget.style.outline = 'none'; e.currentTarget.style.borderColor = emailError ? '#ef4444' : '#3b82f6'; }} disabled={!isEditing} />
            {isEditing && emailError && <div style={styles.errorText}>{emailError}</div>}
          </div>
          <div style={styles.infoItem}>
            <label style={styles.infoLabel}>Дата рождения</label>
            {isEditing ? <input type="date" name="birthDate" value={formData.birthDate} onChange={onInput} style={styles.fieldInput} onFocus={(e) => { e.currentTarget.style.outline = 'none'; e.currentTarget.style.borderColor = '#3b82f6'; }} onBlur={(e) => { e.currentTarget.style.borderColor = '#d1d5db'; }} /> : <div style={styles.fieldDisabled}>{formatDate(userData?.birthDate || '')}</div>}
          </div>
          <div style={styles.infoItem}>
            <label style={styles.infoLabel}>Пол</label>
            {isEditing ? <select name="gender" value={formData.gender} onChange={onInput} style={styles.select} onFocus={(e) => { e.currentTarget.style.outline = 'none'; e.currentTarget.style.borderColor = '#3b82f6'; }} onBlur={(e) => { e.currentTarget.style.borderColor = '#d1d5db'; }}><option value="">Не указан</option><option value="male">Мужской</option><option value="female">Женский</option></select> : <div style={styles.fieldDisabled}>{userData?.gender === 'male' ? 'Мужской' : userData?.gender === 'female' ? 'Женский' : 'Не указан'}</div>}
          </div>
        </div>
        <div style={styles.buttonGroup}>
          {isEditing ? (
            <>
              <button onClick={onSave} disabled={isSaveDisabled || isSaving} style={isSaveDisabled || isSaving ? styles.btnDisabled : styles.btnPrimary}>{isSaving ? 'Сохранение...' : 'Сохранить'}</button>
              <button onClick={onCancel} style={styles.btnSecondary}>Отмена</button>
            </>
          ) : <button onClick={onEdit} style={styles.btnPrimary}><ProfileIcon />Редактировать</button>}
        </div>
      </div>
    </div>
  );
...
const SecuritySection: React.FC<{ styles: ReturnType<typeof createStyles>; onChangePassword: () => void; onTerminateAll: () => void; }> = ({ styles, onChangePassword, onTerminateAll }) => (
  <div style={styles.sectionCard}>
    <div style={styles.sectionHeader}>
    ...
    <div style={styles.sectionBody}>
      <div style={styles.securityItem}>
        <div style={styles.securityItemInfo}><h4 style={styles.securityItemTitle}>Пароль</h4><p style={styles.securityItemDesc}>Последний раз изменён 15 ноября 2025</p></div>
        <button onClick={onChangePassword} style={styles.btnSecondary}>Изменить</button>
      </div>
      <div style={styles.securityItem}>
        <div style={styles.securityItemInfo}><h4 style={styles.securityItemTitle}>Уведомления о входе</h4><p style={styles.securityItemDesc}>Получать уведомления при новых входах в аккаунт</p></div>
        <span style={styles.statusBadge(true)}>Включено</span>
      </div>
      <div style={{ marginTop: 24, paddingTop: 16, borderTop: `1px solid #e5e7eb` }}>
        <button onClick={onTerminateAll} style={styles.btnDanger}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>Завершить все сеансы</button>
      </div>
    </div>
  </div>
);
...
const ActivitySection: React.FC<{
...
}> = ({ sessions, sessionsLoading, sessionsError, currentPage, totalPages, canGoPrev, canGoNext, terminatingSessionId, currentLoginId, styles, isMobile, onTerminateClick, onPageChange, onRetry }) => {
  ...
  return (
    <div style={styles.sectionCard}>
      <div style={styles.sectionHeader}>
        <h2 style={styles.sectionTitle}>Активность</h2>
        <p style={styles.sectionDesc}>История входов и активные сеансы</p>
      </div>
      <div style={styles.sectionBody}>
            ...
            <>
              <div style={styles.tableWrapper}>
                <table style={styles.table}>
                  <thead><tr style={styles.tableHeader}><th style={styles.tableHeaderCell}>Устройство</th><th style={styles.tableHeaderCell}>Вход</th><th style={styles.tableHeaderCell}>Действия</th></tr></thead>
                  <tbody>
                    {sessions.map(session => {
                      const active = isSessionActive(session), current = isCurrent(session), disabled = terminatingSessionId === session.id || !active || current;
                      return (<tr key={session.id} style={{...styles.tableRow,backgroundColor:current?'#dcfce7':undefined}}><td style={styles.tableCell}><div style={styles.sessionInfo}><span style={styles.sessionBrowser}>{session.userAgent}</span></div></td><td style={styles.tableCell}>{formatDateTime(session.loginAt)}</td><td style={styles.tableCell}><button onClick={()=>onTerminateClick(session)} disabled={disabled} style={styles.btnTerminate(disabled)} title={!active?"Сеанс уже завершен":current?"Нельзя завершить текущий сеанс":"Завершить сеанс"}>{terminatingSessionId===session.id?'...':'Завершить'}</button></td></tr>);
                    })}
                  </tbody>
                </table>
              </div>
...
const ScheduleSection: React.FC<{
  scheduleDate: string; 
  scheduleEvents: Array<{id:string,title:string,startTime:string,endTime:string,color:string}>;
  eventsLoading: boolean; 
  styles: ReturnType<typeof createStyles>; 
  isMobile: boolean; 
  onDateChange: (d: string) => void;
}> = ({ scheduleDate, scheduleEvents, eventsLoading, styles, isMobile, onDateChange }) => {
  
  const lanesAssignment = useMemo(() => {
    return assignEventLanes(scheduleEvents.map(e => ({ id: e.id, startTime: e.startTime, endTime: e.endTime })));
  }, [scheduleEvents]);
  ...
  return (
    <div style={styles.sectionCard}>
      <div style={styles.sectionHeader}>
        <h2 style={styles.sectionTitle}>Расписание</h2>
        <p style={styles.sectionDesc}>Встречи и события на выбранный день</p>
      </div>
      <div style={styles.sectionBody}>
        <div style={styles.scheduleControls}>
          <button 
            onClick={() => onDateChange(addDays(scheduleDate, -1))}
          ...
          <button 
            ...
            style={{...styles.btnSecondary, fontSize: 12, padding: '4px 12px'}}
            title="Сегодня"
          >
            Сегодня
          </button>
          ...
          <div style={{...styles.timelineContainer, height: `${timelineContentHeight + 24}px`}}>
            <div style={styles.timelineHours}>
              {Array.from({length:24}, (_,h) => (
                <div key={h} style={styles.timelineHourSlot}>{h.toString().padStart(2,'0')}:00</div>
              ))}
            </div>
                ...
                scheduleEvents.map(ev => {
                  ...
                  return (
                    <div 
                      key={ev.id} 
                      style={{
                        ...styles.timelineEvent(ev.startTime, ev.endTime, ev.color),
        ...
        {scheduleEvents.length > 0 && (
          <div style={{marginTop: 24}}>
            <h4 style={{...styles.sectionTitle, fontSize: 16, marginBottom: 12}}>
              Список встреч ({scheduleEvents.length})
            </h4>
              ...
              {scheduleEvents.map(ev => (
                ...
                >
                  <div style={{flex: 1, minWidth: 0}}>
                    <div style={{fontWeight: 500, color: '#111827'}}>{ev.title}</div>
                    <div style={{fontSize: 12, color: '#6b7280'}}>{ev.startTime} — {ev.endTime}</div>
                  </div>
                </div>
              ))}
            </div>
...
const TerminateModal: React.FC<{isOpen:boolean,sessionInfo:any,onCancel:()=>void,onConfirm:()=>void,terminating:boolean,styles:any,isMobile:boolean}> = ({isOpen,sessionInfo,onCancel,onConfirm,terminating,styles,isMobile}) => !isOpen ? null : (
...
const TerminateAllModal: React.FC<{isOpen:boolean,sessionsCount:number,onCancel:()=>void,onConfirm:()=>void,styles:any}> = ({isOpen,sessionsCount,onCancel,onConfirm,styles}) => !isOpen ? null : (
...
const ChangePasswordModal: React.FC<{modal:any,isChanging:boolean,onChange:(f:any,v:string)=>void,onSubmit:()=>void,onCancel:()=>void,styles:any,validate:(p:string)=>string}> = ({modal,isChanging,onChange,onSubmit,onCancel,styles,validate}) => !modal.isOpen ? null : (
...
type SectionType = 'profile' | 'security' | 'activity' | 'schedule';

export const CabinetPage = () => {
  const { userData, isLoading, updateUserData, getUserSessions, currentLoginId, terminateSession, changePassword } = useCabinet();
  ...
  useEffect(() => {
    const handleResize = () => { const m = window.innerWidth < 768; setIsMobile(m); if (!m) setIsSidebarOpen(true); };
    handleResize(); window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, []);

  useEffect(() => { if (userData) { setFormData({ email: userData.email || '', birthDate: userData.birthDate || '', gender: userData.gender || '' }); setEmailError(''); setIsEmailTouched(false); } }, [userData]);
  useEffect(() => { if (isEmailTouched && formData.email) setEmailError(!isValidEmail(formData.email) ? 'Введите корректный email адрес' : ''); else setEmailError(''); }, [formData.email, isEmailTouched]);

  useEffect(() => {
    if (activeSection === 'schedule') {
      const events = getEventsForDate(scheduleDate);
      setScheduleEvents(events.map(e => ({ 
        id: e.id, 
        title: e.title, 
        startTime: e.startTime, 
        endTime: e.endTime, 
        color: UNIFORM_EVENT_COLOR 
      })));
    }
    if (activeSection === 'activity') loadSessions(currentPage);
    if (isMobile) setIsSidebarOpen(false);
  }, [activeSection, currentPage, scheduleDate]);

  const loadSessions = async (page: number) => {
    setSessionsLoading(true); setSessionsError(null);
    try { const r = await getUserSessions(10, page, false); setSessions(r.logins || []); setTotalSessions(r.totalCount || 0); }
    catch (err) { console.error('Failed to load sessions:', err); setSessionsError('Не удалось загрузить данные о сессиях'); }
  ...
  const terminateAllSessions = async () => { setTerminateAllModal({ isOpen: false }); try { for (const s of sessions.filter(s => s.id !== currentLoginId)) try { await terminateSession(s.id); } catch {} if (currentLoginId) try { await terminateSession(currentLoginId); } catch (e) { throw e; } setSessions([]); setTotalSessions(0); navigate('/sign-in', { state: { message: 'Все сеансы завершены. Войдите снова.' } }); } catch { setSessionsError('Не удалось завершить все сеансы'); loadSessions(currentPage); } };
  ...
  const handleEditClick = () => { if (userData) setFormData({ email: userData.email || '', birthDate: userData.birthDate || '', gender: userData.gender || '' }); setIsEditing(true); setEmailError(''); setIsEmailTouched(false); };
  const handleCancelClick = () => { setIsEditing(false); if (userData) setFormData({ email: userData.email || '', birthDate: userData.birthDate || '', gender: userData.gender || '' }); setEmailError(''); setIsEmailTouched(false); };
  const handleSaveClick = async () => { if (!isValidEmail(formData.email)) { setEmailError('Введите корректный email адрес'); setIsEmailTouched(true); return; } setIsSaving(true); try { await updateUserData(formData); setIsEditing(false); setEmailError(''); setIsEmailTouched(false); } catch (err) { console.error('Failed to save:', err); } finally { setIsSaving(false); } };
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => { const { name, value } = e.target; setFormData(p => ({ ...p, [name]: value })); if (name === 'email' && !isEmailTouched) setIsEmailTouched(true); };
  const handleEmailBlur = () => { setIsEmailTouched(true); if (formData.email && !isValidEmail(formData.email)) setEmailError('Введите корректный email адрес'); };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.layout}>
        <aside style={{...styles.sidebar,left:isMobile?(isSidebarOpen?'0':'-280px'):0}}>
          <div style={styles.profileCard}>
            ...
            <h3 style={styles.profileName}>{userData?.name||'Пользователь'}</h3>
            <p style={styles.profileEmail}>{userData?.email||'Email не указан'}</p>
            <div style={styles.profileMeta}><div style={styles.profileMetaItem}><span>На сайте с</span><span>{formatDate(userData?.registrationDate||'')}</span></div></div>
          </div>
          <nav style={styles.navMenu}>
            {(['profile','security','activity','schedule'] as SectionType[]).map(sec=>(
              <button key={sec} style={styles.navItem(activeSection===sec)} onClick={()=>{setActiveSection(sec);if(isMobile)setIsSidebarOpen(false);}}>
                {renderNavIcon(sec)}
                {sec==='profile'?'Данные':sec==='security'?'Безопасность':sec==='activity'?'Активность':'Расписание'}
              </button>
            ))}
          </nav>
        </aside>
        <main style={styles.contentArea}>
          {isLoading ? <div style={styles.sectionCard}><div style={styles.emptyState}><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="2" style={{animation:'spin 1s linear infinite',margin:'0 auto'}}><circle cx="12" cy="12" r="10"/><path d="M12 2a10 10 0 0 1 10 10"/></svg><p style={{marginTop:12}}>Загрузка...</p></div></div> : activeSection==='profile' ? <ProfileSection userData={userData} isEditing={isEditing} formData={formData} emailError={emailError} isEmailTouched={isEmailTouched} isSaving={isSaving} styles={styles} onEdit={handleEditClick} onCancel={handleCancelClick} onSave={handleSaveClick} onInput={handleInputChange} onEmailBlur={handleEmailBlur} setIsEmailTouched={setIsEmailTouched}/> : activeSection==='security' ? <SecuritySection styles={styles} onChangePassword={handleChangePasswordClick} onTerminateAll={handleTerminateAllClick}/> : activeSection==='schedule' ? <ScheduleSection scheduleDate={scheduleDate} scheduleEvents={scheduleEvents} eventsLoading={eventsLoading} styles={styles} isMobile={isMobile} onDateChange={setScheduleDate}/> : <ActivitySection sessions={sessions} sessionsLoading={sessionsLoading} sessionsError={sessionsError} currentPage={currentPage} totalPages={totalPages} canGoPrev={canGoPrev} canGoNext={canGoNext} terminatingSessionId={terminatingSessionId} currentLoginId={currentLoginId} styles={styles} isMobile={isMobile} onTerminateClick={handleTerminateClick} onPageChange={handlePageChange} onRetry={()=>loadSessions(currentPage)}/>}
        </main>
      </div>
      ...
      <style>{`
        ...
        .timeline-event:hover{filter:brightness(0.95);transform:translateY(-1px);z-index:10}
        
        .timeline-scroll-container::-webkit-scrollbar { width: 6px; height: 6px; }
        .timeline-scroll-container::-webkit-scrollbar-track { background: transparent; border-radius: 4px; }
        .timeline-scroll-container::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 4px; }
        .timeline-scroll-container::-webkit-scrollbar-thumb:hover { background: #94a3b8; }
        .timeline-scroll-container { scrollbar-width: thin; scrollbar-color: #cbd5e1 transparent; }
      `}</style>
    </div>
  );
};
